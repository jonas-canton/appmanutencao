...
Solução: A causa do problema é o fato de o método Delete (além de deletar) mover o ponteiro para o próximo registro disponível. A solução foi passar a executar o método Next somente quando não for dado Delete.
...
Solução: A causa do problema era a grande quantidade de memória consumida armazenando o arquivo num campo blob várias vezes (até estourar o limite). Alterado para armazenar o caminho do arquivo.
...
Solução: Foram armazenados os nomes completos dos arquivos que foram salvos num objeto StringList, e assim que o erro ocorre essa lista é percorrida e o método Delete é chamado passando como argumento o nome completo do arquivo.